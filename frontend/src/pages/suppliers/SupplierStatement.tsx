...
import { FileText, Printer, PlusCircle, ChevronRight, Calendar, ShoppingBag, CheckCircle2, Trash2, Plus, Wallet, Loader2 } from 'lucide-react';
...
export const SupplierStatement: React.FC = () => {
  ...
  const { suppliers, getSupplierLedger, addLedgerEntry } = useCustomerSupplierStore();
  ...
  const ledgerEntries = id ? getSupplierLedger(id) : [];
  ...
  if (!supplier) {
    return (
      <div className="p-8 text-center bg-white rounded-3xl border border-slate-200">
        <p className="text-slate-500 font-bold text-sm">المورد غير موجود</p>
        <Link to="/suppliers" className="text-[#f28913] font-bold text-xs mt-2 inline-block">
  ...
  const handlePaymentSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      addLedgerEntry({
        ...
        description: `سند صرف مباشر (${getMethodLabel(paymentMethod)})`,
        debit: amount,
        credit: 0,
        notes: notes || 'سداد من شاشة كشف الحساب',
      });

      updateBalance(selectedTreasuryId, amount, 'withdrawal');
  ...
  const handleManualEntrySubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      addLedgerEntry({
        ...
        partyId: supplier.id,
        partyName: supplier.name,
        description: manualEntryType === 'discount' ? 'خصم مكتسب / تسوية مدينة (تخفيض الحساب)' : 'إضافة رصيد / تسوية دائنة (زيادة الحساب)',
        debit: manualEntryType === 'discount' ? manualEntryAmount : 0,
        credit: manualEntryType === 'addition' ? manualEntryAmount : 0,
    ...
    try {
      const formattedItems = invoiceItems.map((item) => {
        ...
        return {
          ...
          productCode: prod ? prod.code : 'WOOD-02',
          productName: prod ? prod.name : 'شحنة خشب',
      ...
      await createInvoice(
        {
          ...
          notes: combinedNotes || 'فاتورة توريد جديدة من كشف الحساب',
      ...
      paymentSplits.slice(1).forEach((split) => {
        if (split.amount > 0) {
          updateBalance(split.treasuryId, split.amount, 'withdrawal');
        }
      });
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 print:hidden">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-black text-slate-900 flex items-center gap-2">
              ...
              <span>كشف حساب مورد - {supplier.name}</span>
            </h2>
            <p className="text-xs text-slate-500 mt-0.5">متابعة الفواتير الواردة، المدفوعات والالتزامات المستحقة.</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <button
            ...
            className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2.5 rounded-xl font-bold text-xs shadow-lg shadow-blue-600/20 transition-all"
          >
            <ShoppingBag className="w-4 h-4" />
            <span>+ إضافة فاتورة توريد شحنة جديدة</span>
          </button>
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs flex flex-col sm:flex-row items-center gap-4 print:hidden">
        ...
        <div className="text-xs font-bold text-slate-600">
          إجمالي رصيد المورد المستحق: <span className="text-rose-600 text-sm font-black">{formatArabicNumber(supplier.balance, 0)} ج.م</span>
        </div>
      </div>
      ...
      <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-xs print:shadow-none print:border-none print:p-0 w-full" dir="rtl">
        ...
        <div className="text-center border-b-2 border-black pb-4 mb-6">
          <h1 className="text-2xl font-bold mb-1">كشف حساب مورد / شركة استيراد</h1>
          <h2 className="text-xl font-bold text-slate-900">{supplier.name}</h2>
          {supplier.phone && <p className="text-slate-600 text-sm mt-1">تليفون: {toArabicDigits(supplier.phone)}</p>}
          {supplier.address && <p className="text-slate-500 text-xs">العنوان: {supplier.address}</p>}
        </div>
          ...
          <table className="w-full border-collapse border border-slate-400 text-[13px] print:text-[10px] text-right print:whitespace-nowrap">
            <thead className="bg-slate-100 font-bold">
              <tr>
                ...
                <th className="border border-slate-400 px-3 py-2 text-center">بيان الحركة / تفاصيل الشحنة / وسيلة السداد</th>
                ...
                <th className="border border-slate-400 px-3 py-2 text-center w-28 text-emerald-800">مدفوعاتنا له (دائن)</th>
                <th className="border border-slate-400 px-3 py-2 text-center w-28 text-rose-800">مستحقات الشحنة (مدين)</th>
            ...
            <tbody>
              {filteredEntries.map((row) => {
                ...
                const isInvoiceMainRow = inv && row.credit > 0 && !row.description.includes('دفعة');
                ...
                if (isInvoiceMainRow && inv) {
                      ...
                      <tr className="bg-slate-100/90 font-bold border-b border-slate-400 page-break-inside-avoid">
                        ...
                        <td className="border border-slate-400 px-3 py-2 text-right font-black" colSpan={3}>
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <span className="text-blue-900 bg-blue-50 border border-blue-200 px-2.5 py-0.5 rounded-lg text-xs font-black">
                  ...
                  runningBalanceMath += (row.credit - row.debit);
                  ...
                  return (
                    <tr key={row.id} className="page-break-inside-avoid bg-slate-50/50 hover:bg-slate-100/60">
                      ...
                      <td className="border border-slate-400 px-3 py-2 text-center text-emerald-700 font-extrabold">{row.debit > 0 ? `${formatArabicNumber(row.debit, 0)} ج.م` : '-'}</td>
                      <td className="border border-slate-400 px-3 py-2 text-center text-rose-700 font-bold">{row.credit > 0 ? `${formatArabicNumber(row.credit, 0)} ج.م` : '-'}</td>
          ...
          <div className="bg-white rounded-3xl border border-slate-200 shadow-2xl max-w-2xl w-full p-6 animate-in zoom-in-95 duration-150 space-y-4 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              <h3 className="font-extrabold text-slate-900 text-base flex items-center gap-2">
                ...
                <span>إضافة فاتورة توريد شحنة جديدة للمورد: {supplier.name}</span>
              </h3>
            ...
            <form onSubmit={handleInvoiceSubmit} className="space-y-4 text-xs">
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 bg-blue-50 text-blue-700 hover:bg-blue-100 border border-blue-200 px-3 py-1.5 rounded-xl font-bold transition-all text-xs"
                  >
                ...
                {invoiceItems.map((item, idx) => {
                  ...
                  return (
                    <div key={idx} className="bg-slate-50 p-3 rounded-2xl border border-slate-200 space-y-2.5 relative">
                      <div className="flex items-center justify-between gap-2">
                        <span className="font-bold text-blue-900 bg-blue-50 border border-blue-200 px-2.5 py-0.5 rounded-lg">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                        ...
                        <div>
                          <label className="block font-bold text-slate-700 mb-1">سعر المتر الشراء (ج.م):</label>
                          <input
                            type="number"
                            required
                            min="1"
                      ...
                      <div className="text-left font-bold text-slate-700 text-[11px]">
                        إجمالي الصنف: <span className="text-blue-700 font-black">{formatArabicNumber(itemTotal, 0)} ج.م</span>
                      </div>
                    </div>
                  );
                })}
              ...
              <div className="p-3.5 bg-gradient-to-r from-slate-900 to-[#1e293b] text-white rounded-2xl flex items-center justify-between font-bold">
                <div>
                  <span className="text-xs text-slate-300 block">إجمالي كمية الشحنة: {formatArabicNumber(totalVolumeM3Sum, 4)} m³</span>
                  <span className="text-xs text-blue-400 font-bold">إجمالي قيمة الفاتورة:</span>
                </div>
                <span className="text-lg text-blue-400 font-black">{formatArabicNumber(totalAmountSum, 0)} ج.م</span>
              </div>
              ...
              <div className="space-y-3 p-3 bg-blue-50/50 rounded-2xl border border-blue-200">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-slate-900 text-xs flex items-center gap-1.5">
                    ...
                    <span>تفاصيل طرق وسداد مدفوعات الشحنة:</span>
                  </span>
                  <button
                    type="button"
                    onClick={handleAddPaymentSplit}
                    className="flex items-center gap-1 bg-blue-100 text-blue-800 hover:bg-blue-200 border border-blue-300 px-2.5 py-1 rounded-xl font-bold transition-all text-[11px]"
                  >
                    <Plus className="w-3.5 h-3.5" />
                ...
                {paymentSplits.map((split, sIdx) => (
                  <div key={sIdx} className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 items-end bg-white p-2.5 rounded-xl border border-slate-200">
                    ...
                    <div className="flex items-center gap-2">
                      <div className="grow">
                        <label className="block font-bold text-slate-700 text-[11px] mb-1">خصم من خزانة:</label>
                        <select
                ...
                <div className="flex items-center justify-between text-xs font-bold pt-1">
                  ...
                  <span className="text-slate-600">المتبقي: <strong className="text-rose-600">{formatArabicNumber(currentRemainingAmount, 0)} ج.م</strong></span>
                </div>
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="ملاحظات الشحنة، الميناء، رقم البوليسة..."
                  className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/30"
                />
              </div>
              ...
              <div className="flex items-center justify-end gap-2 pt-3 border-t border-slate-100">
                ...
                >
                  ...
                  <span>{isSubmitting ? 'جاري الحفظ...' : 'تأكيد وتسجيل فاتورة التوريد'}</span>
                </button>
              </div>
            </form>
          </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-3xl border border-slate-200 shadow-2xl max-w-md w-full p-6 animate-in zoom-in-95 duration-150">
            <div className="flex items-center justify-between pb-4 border-b border-slate-100 mb-4">
              <h3 className="font-bold text-slate-900 text-base">سداد دفعة للمورد</h3>
            ...
            <form onSubmit={handlePaymentSubmit} className="space-y-4 text-xs">
              <div>
                ...
                <input
                  ...
                  className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl text-base font-bold text-rose-600 focus:ring-2 focus:ring-[#f28913]/30"
                />
              </div>
              ...
              <div>
                <label className="block font-bold text-slate-700 mb-1">خصم من خزانة</label>
                <select
                ...
                >
                  {treasuries.map((t) => (
                    <option key={t.id} value={t.id}>
                      {t.name} (الرصيد المتاح: {formatArabicNumber(t.balance, 0)} ج.م)
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div>
                <label className="block font-bold text-slate-700 mb-1">ملاحظات السداد</label>
                <textarea
                  rows={2}
                  value={notes}
                  onChange={(e) => setNotes(e.target.value)}
                  placeholder="تحويل انستا باي، شيك بنكي رقم..."
            ...
            <form onSubmit={handleManualEntrySubmit} className="p-5 space-y-4">
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  >
                    خصم مكتسب (تسوية مدينة)
                    <p className="text-[10px] font-normal opacity-80 mt-1">يقلل رصيد المورد</p>
                  </button>
                  <button
                    type="button"
                  ...
                  >
                    إضافة رصيد (تسوية دائنة)
                    <p className="text-[10px] font-normal opacity-80 mt-1">يزيد رصيد المورد</p>
                  </button>
                </div>
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="مثال: خصم خاص، تسوية رصيد قديم..."
                />
              </div>
